...
| 1 / 2 / 4 | set playback speed (1x / 2x / 4x; audio uses `atempo` for pitch-preserved time-stretch) |
| click on the bottom bar | seek video and audio to that timestamp |
...
Pass `--mute` to disable audio entirely. Speed changes use ffmpeg's
`atempo` filter (chained for >2x) for pitch-preserving time-stretch;
if the filter graph fails to build for some reason, falls back
automatically to a chipmunk-style resample-speedup so at least
